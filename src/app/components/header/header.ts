...
@Component({
  ...
  `],
  template: `
<mat-toolbar style="background-image: url('/fondo5.jpg'); background-position: center; display:flex;">
  <button
    class="header-btn"
    mat-icon-button
    matTooltip="Menú"
    ...
    (click)="onToggle.emit()"
    style="color:white;">
    <div style="display:flex;flex-direction:column;justify-content:center;align-items:center;gap:5px;width:20px;height:20px;">
      <span style="display:block;height:2px;width:20px;background:white;border-radius:9999px;"></span>
      <span style="display:block;height:2px;width:20px;background:white;border-radius:9999px;"></span>
      <span style="display:block;height:2px;width:20px;background:white;border-radius:9999px;"></span>
    </div>
  </button>
