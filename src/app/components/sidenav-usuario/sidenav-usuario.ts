...
@Component({
  ...
  template: `
    <mat-sidenav-container class="h-screen" style="background-image: url('/fondo5.jpg');">
      ...
      >
        <div
          class="transition-[width] duration-700 ease-[cubic-bezier(0.25,0.8,0.25,1)] overflow-hidden"
          [style.width.px]="width()"
        >
          <div class="w-full mb-1">
            <img
              [src]="collapsed() ? '/mundial_2026.png' : '/weare26.jpg'"
              [alt]="collapsed() ? 'Logo' : 'MyApp'"
              class=" w-full h-36 object-cover transition-all duration-500"
            />
          </div>

          <ul class="p-2 flex flex-col w-full gap-7 mt-4">
            @for (item of items(); track item.label) {
              <li>
                <a
                  class="hover:bg-[#b8860b] w-full p-2 flex items-center gap-2 rounded-md transition-all duration-300 text-white cursor-pointer"
                  [routerLink]="item.path"
                  routerLinkActive="bg-[#b8860b]"
                  [class.justify-center]="collapsed()"
                >
                  <mat-icon class="!text-[30px] !size-7 shrink-0">{{ item.icon }}</mat-icon>
                  <span
      ...
      <mat-sidenav-content
        [style.margin-left.px]="width()"
        class="transition-[margin] duration-700 ease-[cubic-bezier(0.25,0.8,0.25,1)]"
      >
        <app-header
          [collapsed]="collapsed()"
          (onToggle)="collapsed.set(!collapsed())"
          (onAccount)="abrirEditarPerfil()"
        />
        <router-outlet />
      </mat-sidenav-content>
...
export class SidenavUsuario {
  @ViewChild('dialogEditarPerfil') dialogEditarPerfil!: DialogEditarPerfil;

  items = signal<MenuItem[]>([
    { path: '/usuario/equipos', icon: 'shield', label: 'Equipos' },
    { path: '/usuario/estadios', icon: 'stadium', label: 'Estadios' },
    { path: '/usuario/grupos', icon: 'emoji_events', label: 'Grupos' },
    { path: '/usuario/jugadores', icon: 'sports_soccer', label: 'Jugadores' },
    { path: '/usuario/partidos', icon: 'sports', label: 'Partidos' },
  ]);
